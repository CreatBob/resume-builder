...
前端应按能力展示，而不是把不可用功能置灰留在页面上。主导航能力由 `VITE_APP_FEATURE_MODE` 控制，部署 profile 只代表后端与基础设施组合。

| 能力 | resume-only | full |
| --- | --- | --- |
| 简历编辑 | 显示 | 显示 |
| 模板切换与预览 | 显示 | 显示 |
| PDF / Markdown / JSON 导出 | 显示 | 显示 |
| JSON 导入 | 显示 | 显示 |
| 本地保存 | 显示 | 可显示 |
| 远程多简历保存 | 由 `VITE_RESUME_STORAGE_MODE` 决定 | 由 `VITE_RESUME_STORAGE_MODE` 决定 |
| AI 简历优化 | 保留编辑页内部入口 | 显示 |
| AI 面试 | 隐藏 | 显示 |
| 知识库 / RAG | 隐藏 | 显示 |
| 后端实时语音 | 隐藏 | 显示 |
| 后端健康状态 | 隐藏 | 显示 |
...
- `resume-only` 模式不展示 AI 面试、知识库、后端语音、后端健康状态等后端能力入口；主导航只有简历编辑时不渲染左侧侧边栏。
- `full` 模式展示完整主导航，`spring-ai` 与 `python-ai` 在前端表现上保持一致，不让用户理解为两套不同产品。
...
- 多个一级工作区使用左侧主导航承载；单功能简历编辑模式不渲染左侧主导航。
